      ...
      try {
        ...
        if (entryFromUrl) {
          ...
          try {
            ...
            console.log("[open] Detection result full:", JSON.stringify(det, null, 2));
            if (det?.success && det.userId) {
              userEntry = det.userId;
              console.log("[open] ✅ Using auto-detected entry ID:", userEntry);
            } else {
              console.warn("[open] ⚠️ Detection failed, trying fallback:", det?.error);
              // フォールバック: 一般的なエントリIDを試す（最初の入力フィールド用）
              userEntry = "entry.1000000001"; // よくある最初のエントリID
              console.log("[open] 🔄 Using fallback entry ID:", userEntry);
            }
          } catch (e) {
            console.warn("[open] ⚠️ Entry ID detection failed, using fallback:", e);
            // フォールバック: エントリIDなしでフォームを開く
            userEntry = null;
            console.log("[open] 🔄 Will open form without prefill due to detection failure");
          }
        }
        ...
        let prefill: string;
        if (userEntry) {
          // エントリIDがある場合：prefill付きURL
          prefill = `${viewUrl.split("?")[0]}?usp=pp_url&${userEntry}=${encodeURIComponent(
            profile.userId
          )}`;
          console.log("[open] ✅ Generated prefill URL:", prefill);
        } else {
          // エントリIDがない場合：通常のフォームURL（手動入力必要）
          prefill = viewUrl;
          console.log("[open] ⚠️ Opening form without prefill (manual entry required):", prefill);
        }
        ...
        // 7) 少し待ってからフォームへ遷移
        console.log("[open] 🚀 About to redirect to form:", prefill);
        console.log("[open] 📍 Current location:", location.href);

        setTimeout(() => {
          console.log("[open] 🔄 Executing redirect now...");
          try {
            location.replace(prefill);
          } catch (redirectError) {
            console.error("[open] ❌ Redirect failed:", redirectError);
            setErr(`フォームへの遷移に失敗しました: ${redirectError}`);
          }
        }, 150);
      } catch (e: any) {
